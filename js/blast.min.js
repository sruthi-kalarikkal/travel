...
$(document).ready(function () {
    $(".blast-box").attr("unselectable", "on").css("user-select", "none").on("selectstart", !1), $(".blast-box .blast-icon").on("click", function () {
        ...
        $(".blast-color").eq(a).css("backgroundColor", $(this).text()), $(".blast-color").eq(a).on("click", function () {
            color = $(this).css("backgroundColor"), localStorage.setItem("my_custom_color_travel", color), $(this).css("border", "none").siblings().css("border", "none")
        })
    }), $('input[name="blastCustomColor"]').on("change", function () {
        color = $(this).val(), localStorage.setItem("my_custom_color_travel", color), setColor(color)
    }), $("*").on("click", function () {
        setColor(color)
    })
});
var color, my_custom_color_travel = localStorage.getItem("my_custom_color_travel");
my_custom_color_travel && (color = my_custom_color_travel, setColor(color));